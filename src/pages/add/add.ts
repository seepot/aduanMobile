...
export class AddPage {
  ...
  getData() {
    this.sqlite.create({
      ...
      db.executeSql('CREATE TABLE IF NOT EXISTS aduan(id INTEGER PRIMARY KEY, '+
         ' idkesalahan INTEGER, tarikh TEXT, masa TEXT, lokasi TEXT, latitute TEXT, longitude TEXT,' +
         ' nokenderaan TEXT, catatan TEXT, type TEXT, video TEXT, img TEXT, status INT, arkib INT, usr_id TEXT, no_aduan TEXT)', {})
      .then(res => console.log('berjaya create table'))
      .catch(e => console.log('gagal create table'));
    })
  }
  ...
  saveData() {
    ...
    const value = [this.id,this.form.value.tarikh,this.form.value.masa,this.form.value.lokasi,
      this.location.latitude, this.location.longitude, this.form.value.nokenderaan, 
      this.form.value.catatan, this.type, this.videoCapture, this.lastImage, 
      "1", "0",this.usr_id,""];
      alert(value);
